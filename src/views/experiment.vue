<template>
    ...
    >
          ...
          <v-dialog v-model="dialog" max-width="700px" persistent>
            <template v-slot:activator="{ on, attrs }">
              <v-btn color="success" dark class="mb-2" v-bind="attrs" v-on="on">
                <v-icon> mdi-plus </v-icon> {{ formTitle }}
              </v-btn>
            </template>
            <v-card>
              <v-card-title>
      ...
      <template v-slot:item.actions="{ item }">
        ...
        <v-btn  @click="$router.push({ name: 'experiment2', params: { id: item.id } })" >
          <v-icon >
          mdi-eye
        </v-icon>
        </v-btn>
  ...
  export default {
    ...
    methods: {
       goToJobposting(item) {
        /* this.$router.push(/experiment2/${item.id}) */
        this.$router.push({ name: 'experiment2', params: { id: item.id } })
       },
